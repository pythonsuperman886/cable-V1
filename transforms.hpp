...
namespace transforms{
    ...
    public:
     ...
     #define transforms_Defect std::make_shared<transforms::DefectImpl>
    class DefectImpl : public ComposeImpl{
    private:
        int thickness;
        int diff;
        int min;
...
//        cv::Size size;
    ...
    public:
        DefectImpl(){}
        explicit DefectImpl(int thickness_,int small,int big,bool is_mask=false);
        bool type() override{return CV_MAT;}
        void forward(cv::Mat &data_in, cv::Mat &data_out) override;
        static int myseed;
